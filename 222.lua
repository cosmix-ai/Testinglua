...
IconButton.Image = "rbxassetid://7072718162" -- เปลี่ยนเป็น Asset ID ที่ถูกต้อง
...
MainFrame.Visible = false  -- เริ่มต้นให้ซ่อนไว้
...
if isMobile then
    ...
    RefreshButton.InputBegan:Connect(function(input)
...
local function loadScripts()
    ...
    local success, err = pcall(function()
        ...
        -- ใช้รูปแบบใหม่สำหรับการแยกวิเคราะห์สคริปต์
        for block in raw:gmatch("%-%-%-%s*@name%s*(.-)\n(.-)\n%-%-%-%s*@end") do
            local title, code = block:match("(.-)\n(.*)")
            if title and code then
                table.insert(scripts, {title = title:gsub("%s+$", ""), code = code})
            end
        end
        
        -- หากไม่พบรูปแบบใหม่ ให้ลองใช้รูปแบบเก่า
        if #scripts == 0 then
            for line in raw:gmatch("[^\r\n]+") do
                local title_match = line:match("^title%s*=%s*[\"'](.-)[\"']%s*,%s*script%s*=%s*[\"'](.-)[\"']")
                if title_match then
                    table.insert(scripts, {title = title_match, code = code_match})
                end
            end
        end
        
        -- ถ้ายังไม่พบ ให้ลองรูปแบบอื่น
        if #scripts == 0 then
            for title, code in raw:gmatch("title%s*=%s*[\"'](.-)[\"']%s*,%s*script%s*=%s*[\"'](.-)[\"']") do
                table.insert(scripts, {title = title, code = code})
            end
        end
        
        -- ถ้าไม่พบสคริปต์ใดๆ ให้ใช้สคริปต์ตัวอย่าง
        if #scripts == 0 then
            table.insert(scripts, {
                title = "ตัวอย่างสคริปต์",
                code = "print('Hello from Cosmic Hub!')"
            })
            showStatus("ใช้สคริปต์ตัวอย่าง ตรวจสอบ URL", Color3.fromRGB(200, 150, 50))
        else
        ...
        for i, s in ipairs(scripts) do
            ...
            scriptIcon.Image = "rbxassetid://7072718162"  -- Default icon
...
task.spawn(function()
    wait(1) -- รอสักครู่ก่อนโหลดสคริปต์
    loadScripts()
end)