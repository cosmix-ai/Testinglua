...
local mainFrameWidth = isMobile and 450 or 600
local mainFrameHeight = isMobile and 350 or 400
...
-- Icon button animation and improved drag
local iconDragging = false
local iconDragStart = Vector2.new(0,0)
local iconStartPos = UDim2.new(0,20,0.5,-iconSize/2)
...
UserInputService.InputChanged:Connect(function(input)
    ...
    if iconDragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
        ...
        newX = clamp(newX, 0, viewportSize.X - iconSize)
        newY = clamp(newY, 0, viewportSize.Y - iconSize)
...
local function loadScripts()
    ...
    if not success then
        showStatus("ไม่สามารถโหลดสคริปต์ได้: " .. raw, Color3.fromRGB(200, 50, 50))
        LoadingFrame.Visible = false
        return
    end
...
UserInputService.InputChanged:Connect(function(input)
    if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
        ...
        -- Get screen size
        local viewportSize = workspace.CurrentCamera.ViewportSize
        newX = clamp(newX, -mainFrameWidth/2, viewportSize.X - mainFrameWidth/2)
        newY = clamp(newY, 0, viewportSize.Y - mainFrameHeight)

        MainFrame.Position = UDim2.new(0.5, newX, 0, newY)
    end
end)
...
-- เปิด GUI ทันทีหลังจากโหลดสคริปต์เสร็จ
